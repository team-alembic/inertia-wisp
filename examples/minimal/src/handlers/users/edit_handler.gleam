...
import wisp
import types/user.{type User, type CreateUserRequest, CreateUserRequest}
import data/users
...
fn handle_valid_update_request(req: wisp.Request, id: Int, id_str: String, json_data) -> wisp.Response {
  case users.find_user_by_id(id) {
    Ok(found_user) -> process_update_request(req, id, id_str, found_user, json_data)
    Error(_) -> wisp.not_found()
  }
}

fn process_update_request(req: wisp.Request, id: Int, id_str: String, found_user: User, json_data) -> wisp.Response {
  case decode_user_request(json_data) {
    Ok(user_request) -> handle_decoded_update(req, id, id_str, found_user, user_request)
    Error(_) -> wisp.bad_request()
  }
}

fn handle_decoded_update(req: wisp.Request, id: Int, id_str: String, found_user: User, user_request: CreateUserRequest) -> wisp.Response {
  let errors = user_validator.validate_user_input(user_request.name, user_request.email, option.Some(id))
...
fn handle_update_validation_errors(req: wisp.Request, found_user: User, user_request: CreateUserRequest, errors: dict.Dict(String, String)) -> wisp.Response {
...
fn serialize_user_with_form_data(found_user: User, user_request: CreateUserRequest) -> json.Json {
  json.object([
