...
function TodoList(props: Todo.TodoProps$) {
  ...
  const handleAddTodo = (e: React.FormEvent) => {
    ...
    if (text) {
      ...
      router.post("/todo/add", Todo.encodeAddTodoRequest(request), {
        preserveScroll: true,
        preserveUrl: true,
        only: ["todos"],
  ...
  const handleToggleTodo = (item: Todo.Todo$) => {
    ...
    const encoded = Todo.encodeToggleTodoRequest(request);

    router.post("/todo/toggle", encoded as any, {
      only: ["todos"],
      preserveUrl: true,
      preserveScroll: true,
    });
...
export default decodeProps(TodoList, Todo.decodeTodoProps());
