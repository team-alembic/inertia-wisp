...
pub fn dashboard_handler(
  ...
  |> inertia.prop(dashboard.system_status("All systems operational"))
  // Core metrics - included by default for dashboard overview
  |> inertia.prop(dashboard.user_count(1247))
  |> inertia.prop(dashboard.post_count(89))
  // Detailed data - only loaded when admin specifically requests it (potentially expensive query)
  |> inertia.prop(
    dashboard.recent_signups(fn() {
