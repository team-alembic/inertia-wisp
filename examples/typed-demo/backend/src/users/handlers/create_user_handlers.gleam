...
pub fn create_user_page_handler(
  ...
  |> inertia.assign_prop_t(home.message("Fill out the form below to create a new user account."))
...
pub fn create_user_handler(
  ...
  use request <- inertia.require_json(
    ctx,
    users.create_user_request_decoder(),
  )
  
  // Validate the request
  let validation_errors = validators.validate_create_user_request(request.name, request.email, request.bio)
