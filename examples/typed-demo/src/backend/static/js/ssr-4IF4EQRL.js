...
function render(page) {
  return W({
    page,
    render: import_server.default.renderToString,
    resolve: async (name) => {
      try {
        const component = await globImport_tsx(`./${name}.tsx`);
        return component.default || component;
      } catch (error) {
        console.error(`SSR: Component '${name}' not found:`, error);
        return () => import_react.default.createElement("div", {}, `Component '${name}' not found`);
      }
    },
    setup: ({ App, props }) => import_react.default.createElement(App, props)
  });
}
export {
  render
