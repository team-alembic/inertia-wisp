...
pub fn setup_ssr_example() {
  ...
  let ssr_config =
    types.SSRConfig(
      enabled: True,
      path: "priv",
      // Directory containing your ssr.js file
      module: "ssr",
      // Module name (ssr.js exports)
      pool_size: 4,
      // Number of Node.js worker processes
      timeout_ms: 5000,
      // Render timeout
...
pub fn ssr_decision_flow_example() {
  ...
  let ctx =
    ...
    |> inertia_gleam.assign_lazy_prop("computed", fn() {
      json.int(42)
      // This will be evaluated before SSR decision
    })
...
pub fn production_ssr_example() {
  ...
  let prod_config =
    ...
    |> config.with_timeout(3000)
    // Shorter timeout for better UX
    |> config.with_path("/app/priv")
